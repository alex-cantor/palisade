...
The first step of the project--the main engine of the tool--is the mock environment generator. So, what does that consist of? Spawning VMs and injecting vulnerabilities, largely. So, let's take a look at spawning VMs.

There are two main ways we can spawn VMs in ProxMox (PVE): we can build them from the iso, or we can clone an existing VM. Cloning an existing VM is much easier for a few reasons: we don't need to script building the VMs and it is *super* reliable. For our purposes, we'll clone from a template, which is functional and ready for our use.

## Choosing OSs to Clone

I knew I wanted to be able to prepare my team for anything, from old to new, from common to esoteric. So, I formed a list of the following OSs/distros which I wanted to make a template of (or, as says my new favorite verb, templatize):
...
This process was a good bit annoying (especially for Windows, not so much for Linux, only a little bit for FreeBSD)--but I did learn a ton, so I guess it all evens out in the end.
...
qm create 9001 --name tmpl-debian12 --memory 2048 --cores 2 --net0 virtio,bridge=vmbr0
...
> Note: `vmbr0` is my bridge name on my PVE host and `local-lvm` is my storage pool; while I believe they are likely the same, yours may differ, so check Datacenter --> Storage in the PVE UI to confirm!

Of course, it would be a pain to actually run these commands individually each time (especially for this tool) so it all goes through the Django command `python manage.py create_pve_templates` (which doesn't only do this for one VM, but does it for all selected VMs--more on this in a bit).

Now for the interesting part of this section, let's break down what each command is actually doing:
...
2. `qm create 9001 --name tmpl-debian12 --memory 2048 --cores 2 --net0 virtio,bridge=vmbr0` - now we are getting into more advanced commands, although this is still quite simple. `qm` is a built-in tool in PVE which is incredibly valuable for managing VMs. Here, we use it to create a VM with the ID 9001, the name tmpl-debian12, 2048 MB (2 GB) of memory, two CPU cores, and configures the first network adapter (net0) to use the VirtIO paravirtualized network driver (which is very performant), and bridges the network driver to vmbr0 (the name of my bridge — yours may be different)
3. `qm set 9001 --scsihw virtio-scsi-pci --scsi0 local-lvm:vm-9001-disk-0` - this sets the SCSI controller hardware type to VirtIO SCSI PCI (a high-performance storage controller standard for PVE), and attaches a specific virtual disk to the VM's first SCSI slot (scsi0). The disk name format `vm-9001-disk-0` is important — PVE expects it in exactly this form.
4. `qm set 9001 --ide2 local-lvm:cloudinit` - `--ide2` specifies the cloud-init drive should be on IDE2 interface slot (IDE because it is universally compatible so you don't need special VirtIO drivers); `local-lvm:cloudinit` tells PVE to generate a special cloudinit iso on local-lvm
...
Windows was really annoying. The TL;DR here is that you can use autounattend.xml and unattend.xml for a speedy setup--but I did not know about those things originally. I am moving on at this point (as I have been working on the templatization + writeup for a few weeks now), but I will come back eventually (in a later blog post).
...
def wait_for_task(prox, node, task_id, timeout=1800):
...
- Templates: 9000–9099
- Baseline VMs: 49000–49999 (the base copy on which you can add injects, then this is copied to all other teams when you are ready)
- Competition VMs: `50000 + (team_number * 100) + offset`
